...
public class DirectoryActions extends HttpServlet {
    ...
    private String LoadFile(int id, String username){
        ...
        try{
            ...
            stmt = conn.prepareStatement("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=\"ChatMore\"");
    ...
    private boolean CreateProject(String project_name, String username){
        ...
        try{
            ...
            DataSource datasource = (DataSource) ctx.lookup("java:comp/env/jdbc/mysql");
            conn = datasource.getConnection();
            stmt = conn.prepareStatement("CREATE TABLE IF NOT EXISTS `ChatMore`.`" + username + Values.ProjectDS + project_name + "` (`name` VARCHAR(50) NOT NULL, `type` INT NOT NULL, `content` BLOB NULL, `parent_id` INT NOT NULL, `id` INT  NOT NULL AUTO_INCREMENT, PRIMARY KEY (`id`, `parent_id`));");
            stmt.execute();
        }catch(Exception e){
            e.printStackTrace();
            result = false;
    ...
    private boolean DeleteProject(String project_name, String username){
        ...
        try{
            ...
            DataSource datasource = (DataSource) ctx.lookup("java:comp/env/jdbc/mysql");
            conn = datasource.getConnection();
            stmt = conn.prepareStatement("DROP TABLE IF EXISTS `ChatMore`.`" + username + Values.ProjectDS + project_name + "`");
            stmt.execute();
        }catch(Exception e){
            e.printStackTrace();
            result = false;
